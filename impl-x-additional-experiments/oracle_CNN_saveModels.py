...
SAVEFILE = 'results_oracle-Combined-model.txt'
...
def performCrossValidation(x_data, y_data):
    ...
    for seed in range(ROUNDS):
        ...
        for train, test in kfold.split(x_data, y_data):
            print('Training Baseline model %i/%i' % (percentage_done+1, KFOLDS*ROUNDS))
    ...
    for seed in range(ROUNDS):
        ...
        kfold = KFold(n_splits=KFOLDS, shuffle=True, random_state=seed)
        for train, test in kfold.split(x_data, y_data):
            print('Training Pipeline model %i/%i' % (percentage_done+1, KFOLDS*ROUNDS))
            from keras import backend as K
            K.clear_session()
            # tf.reset_default_graph()
            ...
            model = text_cnn_model_appraisals(MAX_SEQUENCE_LENGTH, vocab_size,
                        ...
                        embedding_matrix, DROPOUT, len(DIMENSIONS), 'sigmoid')
    ...
    for seed in range(ROUNDS):
        ...
        for train, test in kfold.split(x_data, y_data):
            print('\n\n############################################################')
            print('Evaluating fold %i/%i in run %i/%i' % ((percentage_done+1)%10, KFOLDS, seed+1, ROUNDS))
            print('############################################################')
            from keras import backend as K
            K.clear_session()
            tf.reset_default_graph()
            ...
            appraisal_emotion_predictor = load_model('saved_models/dim_to_emotion_' + str(percentage_done))
